...
if (!dirname.startsWith('/')) dirname = process.cwd() + '/' + process.argv.slice(2);
...
for (const [key, component] of Object.entries(all_components)) {
  ...
  if (
    ...
    details =
      ...
      component['classProperties'].length +
      '; Number of methods: ' +
      classMethods;
    csv_smells.push(
      createSmell(component['file_url'], out_component['Component'], 'Large Component', details)
    );
    out_component['LC'] = 'Y';
  } else out_component['LC'] = 'N';

  if (component['properties'].length > thresholds['N_props']) {
    has_smells = true;
    details =
      'Number of props: ' + component['properties'].length + '; Props: ' + component['properties'];
    csv_smells.push(
      createSmell(component['file_url'], out_component['Component'], 'Too many props', details)
    );
    out_component['TP'] = 'Y';
  } else out_component['TP'] = 'N';
  ...
  if (component['forceUpdate'].length > 0) {
    ...
    csv_smells.push(
      createSmell(component['file_url'], out_component['Component'], 'Force Update', details)
    );
    out_component['FU'] = component['forceUpdate'].length;
  } else out_component['FU'] = 0;
  ...
  if (component['propsInitialState'].length > 0) {
    ...
    for (var propsInitialState of component['propsInitialState'])
      details +=
        'Lines ' +
        propsInitialState['line_start'] +
        ' - ' +
        propsInitialState['line_end'] +
        ': \n' +
...
if (output_files.length > 0) {
  const output_files_t = output_files.reduce((out, { id, ...x }) => {
    out[id] = x;
    return out;
  }, {});
  console.table(output_files_t, [
    'id',
    'Large File',
    'LOC',
    'N_Components',
...
} else console.log('There are no Large Files!');

if (output_components.length > 0) {
  const output_components_t = output_components.reduce((out, { id, ...x }) => {
    out[id] = x;
    return out;
  }, {});
  ...
  console.log(
    'Code smells (LC: Large component, TP:Too many props, IIC: Inheritance insteadof Composition; PIS: props in Initial State; DOM: Directly DOM manipulations; JSX: JSX outside the render method; FU: Force update; UC: Uncontrolled component)\n'
  );

  csvWriter('components_smells.csv', csv_smells, Object.keys(csv_smells[0]), 'components');
} else console.log('There are no components with smells');
