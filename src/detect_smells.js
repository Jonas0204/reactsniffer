...
function recursive_search(item, component, components, functions) {
  ...
  for (var [key, value] of Object.entries(item)) {
    ...
    if (!value) continue;
      ...
      if (
        (value == 'ClassDeclaration' || value == 'FunctionDeclaration') &&
        item['id'] &&
        item['id']['name'][0] == item['id']['name'][0].toUpperCase()
      ) {
        ...
        if (
          ...
          item['superClass'] &&
          'name' in item['superClass'] &&
          item['superClass']['name'] != 'PureComponent' &&
          item['superClass']['name'] != 'Component'
        ) {
      ...
      // Looking for ArrowFunctions Components
        ...
        item['declarations'][0]['id']['name'][0] ==
          item['declarations'][0]['id']['name'][0].toUpperCase()
      ) {
        ...
        component['loc'] =
          arrowFunction['loc']['end']['line'] - arrowFunction['loc']['start']['line'] + 1;
      ...
      //Check whether there are functions that are not components neither class methods
        ...
        if (value == 'FunctionDeclaration') {
          if (item['id']) functionName = item['id']['name'];
          else functionName = 'Noname';
        } else {
          functionName = item['declarations'][0]['id']['name'];
        }

        if (!('name' in component)) functions.push(functionName);
        else component['functions'].push(functionName);
        ...
        else component['classProperties'].push(item['key']['name']);
        ...
        if (item['kind'] == 'constructor') {
          ...
          for (const expression of item['body']['body']) {
            if (check_props_initial_state(expression, params)) {
              ...
              propsInitialState['line'] = read_files.get_lines(
                component['file_url'],
    ...
    // Check for forceUpdate and directy dom manipulation
      ...
      if (value['property'] && value['property']['name']) {
        if (value['property']['name'] == 'forceUpdate' || value['property']['name'] == 'reload') {
          ...
          forceUpdate['line'] = read_files.get_line(
            component['file_url'],
            forceUpdate['line_number']
          );

          component['forceUpdate'].push(forceUpdate);
        } else if (
          value['object']['name'] &&
          (value['object']['name'] == 'document' || value['object']['name'] == 'element')
        )
          if (dom_elements().includes(value['property']['name'])) {
            dom_manipulation = {};
            dom_manipulation['line_number'] = value['loc']['start']['line'];
            dom_manipulation['line'] = read_files.get_line(
              component['file_url'],
              dom_manipulation['line_number']
            );
      ...
      if (item['attributes']) {
        for (var [k, attr] of Object.entries(item['attributes'])) {
          if (attr['name'] && attr['name']['type'] == 'JSXIdentifier')
            if (attr['name']['name'] == 'ref') attr_ref = true;
            else if (attr['name']['name'] == 'value') attr_value = true;
        }

        if (attr_ref && !attr_value) {
          uncontrolled = {};
          uncontrolled['line_number'] = value['loc']['start']['line'];
          uncontrolled['line'] = read_files.get_line(
            component['file_url'],
            uncontrolled['line_number']
          );
